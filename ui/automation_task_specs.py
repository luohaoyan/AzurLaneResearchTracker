...
_TASK_DEFINITIONS: Dict[str, AutomationTaskDefinition] = {
    ...
    "adb_auto_connect": AutomationTaskDefinition(
        key="adb_auto_connect",
        title="模拟器自动连接",
        kind=TaskStateKind.AUTO_TESTING,
        start_message="正在自动发现并连接当前模拟器。",
        bridge_method="run_adb_auto_connect",
        feature_key="adb_connection_check",
        button_text="自动连接模拟器",
    ...
    "game_auto_login": AutomationTaskDefinition(
        ...
        title="游戏自动登录",
        kind=TaskStateKind.AUTO_TESTING,
        start_message="正在尝试启动碧蓝航线并进入主页。",
        bridge_method="run_game_auto_login",
        feature_key="adb_connection_check",
        button_text="测试游戏启动",
        summary="扫描模拟器中已安装的碧蓝航线客户端，并尝试自动选服、关闭公告后进入港区主页。",
        cancel_supported=True,
    ),
    "game_enter_home": AutomationTaskDefinition(
        key="game_enter_home",
        title="进入游戏主页",
        kind=TaskStateKind.AUTO_TESTING,
        start_message="正在从模拟器桌面进入港区主页。",
        bridge_method="run_game_enter_home",
        feature_key="adb_connection_check",
        button_text="进入游戏主页",
        summary="以更宽松的等待时间执行完整自动登录链路，适合验证从桌面到港区主页的闭环。",
        cancel_supported=True,
    ),
    ...
    "design_chart_flow_test": AutomationTaskDefinition(
        key="design_chart_flow_test",
        title="设计图功能测试",
        kind=TaskStateKind.AUTO_TESTING,
        start_message="正在执行设计图完整流程。",
        bridge_method="run_design_chart_flow",
        feature_key="design_chart_flow_test",
        button_text="测试筛选",
        summary="自动切到设计图页、按稀有度跑一遍现有流程，并自动尝试读取上一次 summary 作为断点续跑游标。",
        cancel_supported=True,
    ),
    "design_fragment_scan": AutomationTaskDefinition(
        key="design_fragment_scan",
        title="设计图扫图识别",
        kind=TaskStateKind.OCR_PROCESSING,
        start_message="正在采集并识别设计图碎片。",
        bridge_method="run_design_fragment_scan",
        feature_key="design_fragment_scan",
        button_text="扫图识别",
        summary="按指定稀有度从当前设计图页分帧采集，执行 OpenCV + OCR + ONNX/PyTorch assist，并输出 JSON/CSV。",
        cancel_supported=True,
    ),
