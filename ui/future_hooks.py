...
║                                                                  ║
║   【一句话解释】提前定义自动化、OCR、模拟出货等未来页面的挂接口。║
║   【类比理解】它像港区船坞的预留泊位，功能船只做好后可直接停靠。║
║   【数据流说明】FutureFeatureSpec → FeatureHookRegistry → UI 页。║
╚══════════════════════════════════════════════════════════════════╝
"""
...
class FutureFeatureSpec:
    ...
    输入：
        ...
        title: 页面展示标题。
        summary: 功能说明。
        status: 当前状态，例如 planned / experimental / ready。
        entry_point: 未来真实实现的 Python 入口字符串。
    ...
    def _default_features() -> List[FutureFeatureSpec]:
        ...
        return [
            FutureFeatureSpec(
                ...
                title="模拟器自动采集",
                summary="连接 ADB、截图、点击并把识别结果送入数据录入页。",
                status="planned",
            ...
            FutureFeatureSpec(
                key="ocr_recognition",
                title="装备数字识别",
                summary="使用 OpenCV 与 PaddleOCR 识别装备数量和碎片数量。",
                status="planned",
            ...
            FutureFeatureSpec(
                ...
                title="科研出货模拟",
                summary="按用户配置模拟科研产出，生成彩装/金装的期望分布。",
                status="planned",
            ...
            FutureFeatureSpec(
                key="luck_prediction",
                title="欧非走势预测",
                summary="基于历史记录和模拟出货结果，预测后续欧非值变化区间。",
                status="planned",
            ...
            FutureFeatureSpec(
                ...
                title="港区动效资源",
                summary="预留 GIF / APNG / 序列帧资源入口，用于页面待机动画和反馈动效。",
                status="planned",
