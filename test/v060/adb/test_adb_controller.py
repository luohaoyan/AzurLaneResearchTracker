...
def test_auto_connect_simulator_returns_int_result_contract() -> None:
    """自动连接入口应返回 INT 版本的完整结果契约。"""
    controller = _controller(serial=None)
    device = AdbDevice("127.0.0.1:7555", "device")

    controller.check_connection = lambda serial=None, task_context=None, reconnect=False: AdbConnectionResult(
        True,
        "ready",
        "ADB 设备连接正常。",
    ...
    result = controller.auto_connect_simulator("mumu", serial="127.0.0.1:7555", max_candidates=1)

    assert result.success is True
    assert result.status == "ready"
    assert result.selected_device is not None
    assert result.selected_device.serial == "127.0.0.1:7555"
    assert result.attempted_serials == ()
    assert result.to_payload()["device_serial"] == "127.0.0.1:7555"
    assert "commands" in result.to_payload()
