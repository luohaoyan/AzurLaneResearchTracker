...
class DesignStateProbe:
    """按调用次数返回不同仓库设计图状态，方便测试设计页切换。"""
    ...
    def __call__(self, candidate: object = None) -> dict[str, object]:
        ...
        self.calls += 1
        return {
            "screen_state": self.states[index],
            "scene_hint": "design_tab",
            "confidence": 0.99,
        }
...
class FakeWarehouseDesignController(FakeEquipmentController):
    """专门用于验证设计图页切换与截图确认的 fake 控制器。"""

    def select_warehouse_tab(self, tab: str, state_probe: Callable[..., object], **kwargs: Any) -> NavigationResult:
        """模拟切到设计图页并落一张可被识别器读取的截图。"""
        self.select_tab_calls.append(tab)
        try:
            state_probe({"screen_state": f"warehouse_{tab}", "scene_hint": f"{tab}_tab"})
        except TypeError:
            state_probe()
        screenshot_path = PathManager.get_work_dir() / f"{tab}_confirmed.png"
        screenshot_path.parent.mkdir(parents=True, exist_ok=True)
        screenshot_path.write_bytes(self.screenshot_bytes[0])
        return NavigationResult(
            True,
            "ready",
            "仓库标签切换完成。",
...
def test_ensure_warehouse_design_page_ready_confirms_design_tab(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """仓库页切到设计图标签后，应能通过仓库标签识别结果确认。"""
    monkeypatch.setattr(PathManager, "get_work_dir", lambda: tmp_path / "workdir")
    monkeypatch.setattr("core.automation.equipment_page.equipment_page_adb_api.time.sleep", lambda *_args, **_kwargs: None)
    api = EquipmentPageAdbApi()
    fake = FakeWarehouseDesignController()
    api._controller_factory = lambda config: fake
    api._warehouse_label_detector = None
    probe = DesignStateProbe(("warehouse_design",))

    class FakeLabelDetector:
        """返回设计图页命中的仓库标签识别器。"""

        def detect(self, screenshot: str | Path, sort_templates: Any = None) -> WarehouseLabelResult:
            """生成设计图页确认结果。"""
            ...
            return WarehouseLabelResult(
                ...
                detections=(
                    WarehouseLabelDetection("tab_design", (787, 651, 155, 54), True, 0.99, "selected", 0.98, "tab"),
                ),
                warnings=(),
            )

    result = api.ensure_warehouse_design_page_ready(state_probe=probe, label_detector=FakeLabelDetector())

    assert result.success is True
    assert result.status == "ready"
    assert result.payload is not None
    assert result.payload["target_tab"] == "design"
    assert result.payload["design_tab_confirmed"] is True
    assert result.payload["design_tab_state"] == "design"
    assert result.payload["warehouse_label_result"]["page_type"] == "design"
    assert fake.select_tab_calls == ["design"]
    assert probe.calls >= 1


def test_ensure_warehouse_design_page_ready_rejects_unknown_page_type(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """如果识别器没确认到 design 页，结果应明确返回 not_confirmed。"""
    monkeypatch.setattr(PathManager, "get_work_dir", lambda: tmp_path / "workdir")
    monkeypatch.setattr("core.automation.equipment_page.equipment_page_adb_api.time.sleep", lambda *_args, **_kwargs: None)
    api = EquipmentPageAdbApi()
    fake = FakeWarehouseDesignController()
    ...
    class FakeUnknownLabelDetector:
        """返回未知页面类型的仓库标签识别器。"""
    ...
    result = api.ensure_warehouse_design_page_ready(state_probe=probe, label_detector=FakeUnknownLabelDetector())

    assert result.success is False
    assert result.status == "not_confirmed"
    assert result.payload is not None
    assert result.payload["design_tab_confirmed"] is False
    assert fake.select_tab_calls == ["design"]
