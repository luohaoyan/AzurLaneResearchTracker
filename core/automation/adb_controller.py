...
class AdbController:
    ...
    def auto_connect_simulator(
        ...
        *,
        ...
        candidate_serials = tuple(dict.fromkeys(normalize_serial(item) for item in base_candidates if normalize_serial(item)))
        ...
        connect_timeout = min(self.command_timeout, float(self.adb_config.get("auto_connect_timeout", 2.0) or 2.0))
        ...
        for candidate in candidate_serials:
            ...
            command = self.run_adb(["connect", candidate], adb_path=resolution.adb_path, timeout=connect_timeout)
    ...
    def _safe_detect_simulator_profiles(
        ...
        """安全读取当前 ADB 设备指纹，失败时返回空列表而不打断连接结果。"""
        try:
