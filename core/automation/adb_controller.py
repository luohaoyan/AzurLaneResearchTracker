...
@dataclass(frozen=True)
class AdbAutoConnectResult:
    """
    模拟器自动连接结果。
    输入：
        success/status/message/selected_device/attempted_serials。
    输出：
        GUI 可展示当前连接状态、候选端口和自动连接命令。
    使用示例：
        result = controller.auto_connect_simulator("mumu")
    """
    ...
    selected_device: Optional[AdbDevice] = None
    candidates: Tuple[AdbDevice, ...] = ()
    simulator_profiles: Tuple[AdbSimulatorProfile, ...] = ()
    attempted_serials: Tuple[str, ...] = ()
    adb_path: Optional[str] = None
    adb_source: str = "missing"
    warnings: Tuple[str, ...] = ()
    command_results: Tuple[AdbCommandResult, ...] = ()
    ...
    def to_payload(self) -> Dict[str, Any]:
        """转换为 GUI 状态面板可直接读取的 payload。"""
        return {
            "adb_path": self.adb_path,
            "adb_source": self.adb_source,
            "adb_path_exists": bool(self.adb_path),
            "device_serial": self.selected_device.serial if self.selected_device else None,
            "device_state": self.selected_device.state if self.selected_device else None,
            "connection_status": self.status,
            "candidates": [device.to_dict() for device in self.candidates],
            "detected_simulators": [profile.to_dict() for profile in self.simulator_profiles],
            "attempted_serials": list(self.attempted_serials),
            "commands": [item.to_dict() for item in self.command_results],
        }
...
class AdbController:
    ...
    def configured_serial(self) -> Optional[str]:
        ...
        if explicit_serial:
            return normalize_serial(explicit_serial)
        return None
    ...
    def _simulator_type_for(
        ...
        joined_keys = " ".join(props.keys())
        if any(token in joined for token in ("ldplayer", "leidian", "leidianbox", "dnplayer")) or "ldinit" in joined_keys:
    ...
    def auto_connect_simulator(
        ...
        *,
        ...
        """
        按 INT worktree 的稳定实现自动尝试 ADB 连接。

        输入：
            simulator_key: 当前配置的模拟器 key。
            task_context: 可选任务上下文。
            include_all_profiles: 当前配置失败后是否追加其他模拟器端口族。
            serial: 可选手动 serial，优先尝试。
            max_candidates: 最大尝试数量，避免 UI 长时间等待端口扫描。
        输出：
            AdbAutoConnectResult，保留候选、尝试命令和最终设备。
        使用示例：
            result = controller.auto_connect_simulator("leidian", serial="127.0.0.1:5555")
        """
        self._raise_if_cancelled(task_context, "模拟器自动连接已取消。")
        resolution = self.find_adb()
        if not resolution.adb_path:
        ...
        preferred_serial = normalize_serial(serial or self.configured_serial or "")
        base_candidates = list(
            build_auto_connect_candidates(
                simulator_key or str(self.simulator_config.get("type", "") or ""),
                self.simulator_config,
                include_all_profiles=include_all_profiles,
            )
        )
        if preferred_serial:
            base_candidates.insert(0, preferred_serial)
        candidate_serials = tuple(
            dict.fromkeys(normalize_serial(item) for item in base_candidates if normalize_serial(item))
        )
        configured_limit = self.adb_config.get("max_auto_connect_candidates", 12)
        safe_limit = int(max_candidates if max_candidates is not None else configured_limit or 12)
        safe_limit = max(1, min(64, safe_limit))
        candidate_serials = candidate_serials[:safe_limit]
        connect_timeout = min(
            self.command_timeout,
            float(self.adb_config.get("auto_connect_timeout", 2.0) or 2.0),
        )

        before = self.check_connection(serial=preferred_serial or None, task_context=task_context)
        if before.success and before.selected_device is not None:
            profiles = self._safe_detect_simulator_profiles(task_context)
            return AdbAutoConnectResult(
                True,
                "ready",
                f"模拟器已连接：{before.selected_device.serial}",
                selected_device=before.selected_device,
                candidates=before.candidates,
                simulator_profiles=profiles,
                attempted_serials=(),
                adb_path=before.adb_path or resolution.adb_path,
                adb_source=before.adb_source or resolution.source,
                warnings=before.warnings,
                command_results=(before.command_result,) if before.command_result else (),
            )
        if before.status == "multiple_devices" and not preferred_serial:
            profiles = self._safe_detect_simulator_profiles(task_context)
            return AdbAutoConnectResult(
                False,
                "multiple_devices",
                "发现多台 ADB 设备，请在配置中指定 serial 后再自动连接。",
                candidates=before.candidates,
                simulator_profiles=profiles,
                attempted_serials=(),
                adb_path=before.adb_path or resolution.adb_path,
        ...
        command_results: list[AdbCommandResult] = []
        attempted: list[str] = []
        last_candidates = before.candidates
        warnings = list(before.warnings)
        if before.message:
            warnings.append(before.message)

        for candidate in candidate_serials:
            self._raise_if_cancelled(task_context, "模拟器自动连接已取消。")
            if ":" not in candidate:
                warnings.append(f"跳过非 TCP serial，等待 adb devices 自动发现: {candidate}")
                continue
            ...
            device_list = self.list_devices(task_context)
            last_candidates = device_list.devices
            selected = self._select_device(device_list.devices, candidate) if device_list.success else {
                "success": False,
                "status": device_list.status,
            ...
            if selected["success"]:
                ...
                return AdbAutoConnectResult(
                    True,
                    "ready",
                    f"模拟器自动连接成功：{candidate}",
                    selected_device=device,
                    candidates=device_list.devices,
                    simulator_profiles=profiles,
                    attempted_serials=tuple(attempted),
                    adb_path=device_list.adb_path or resolution.adb_path,
                    adb_source=device_list.adb_source or resolution.source,
                    warnings=tuple(dict.fromkeys(warnings)),
                    command_results=tuple(command_results),
                )

        final_connection = self.check_connection(serial=preferred_serial or None, task_context=task_context)
        profiles = self._safe_detect_simulator_profiles(task_context)
        status = final_connection.status if final_connection.status != "unavailable" else "not_connected"
        message = "未能自动连接模拟器，请确认模拟器已启动并开启 ADB 调试。"
        if final_connection.status == "multiple_devices":
            message = "发现多台 ADB 设备，请在配置中指定 serial 后再自动连接。"
        elif final_connection.status == "unauthorized":
            message = "设备未授权，请在模拟器或设备中允许 ADB 调试授权。"
        elif not candidate_serials:
            message = "没有可用的模拟器 serial 候选，请在模拟器配置中填写 adb.port 或 adb.serial。"
        if final_connection.message:
            warnings.append(final_connection.message)
        return AdbAutoConnectResult(
            False,
            status,
            message,
            selected_device=final_connection.selected_device,
            candidates=final_connection.candidates or last_candidates,
            simulator_profiles=profiles,
            attempted_serials=tuple(attempted),
            adb_path=final_connection.adb_path or resolution.adb_path,
            adb_source=final_connection.adb_source or resolution.source,
            warnings=tuple(dict.fromkeys(warnings)),
            command_results=tuple(command_results),
        )

    def _safe_detect_simulator_profiles(
        self,
        task_context: Optional[TaskExecutionContext] = None,
    ) -> Tuple[AdbSimulatorProfile, ...]:
        """安全读取当前 ADB 设备指纹，失败时不打断连接结果。"""
        try:
            detected = self.detect_simulators(task_context=task_context)
        except Exception as exc:
            self.logger.warning(f"模拟器指纹读取失败，已忽略: {exc}")
            return ()
        return detected.simulators if detected.success else ()
    ...
    def start_app(
        ...
        *,
        ...
        result = self.run_adb(args, serial=serial or self.configured_serial, timeout=self.command_timeout)
        # 部分 Android/模拟器版本即使 Activity 不存在也可能返回 0，
        # 但会在 stdout/stderr 输出明确错误；将其归类为失败才能触发
        # login_game 的 monkey 主入口回退。
        if activity_name and result.success:
            output = f"{result.stdout}\n{result.stderr}".lower()
            activity_error_markers = (
                "error type",
                "activity class",
                "unable to resolve intent",
                "does not exist",
            )
            if any(marker in output for marker in activity_error_markers):
                return AdbCommandResult(
                    False,
                    "activity_not_found",
                    "指定 Activity 不存在，准备回退到应用主入口。",
        ...
        *,
        ...
        for attempt in range(retries + 1):
            ...
            if not start_result.success:
                ...
                if activity_name:
                    fallback_result = self.start_app(
                        package,
                        None,
                        serial=device_serial,
                        task_context=task_context,
                    ...
                    if fallback_result.success:
                        warnings.append("指定 Activity 启动失败，已回退到主入口 monkey 启动。")
                        start_result = fallback_result
                    else:
                        warnings.append(f"主入口回退启动也失败: {fallback_result.status}")
                        continue
                else:
                    continue
            ...
            for index, step in enumerate(steps, start=len(all_step_results) + 1):
                ...
                # 登录页上的公告、服务器标签和文字按钮可能尚未出现（或 OCR
                # 不可用）。这些辅助步骤声明 optional 时只记录 warning，
                # 不应阻断后续的前台应用/状态探针检查。
                if (
                    isinstance(step, dict)
                    and bool(step.get("optional", False))
                    and not step_result.success
                ):
                    step_result = AdbOperationStepResult(
                        index,
                        step_result.action,
                        True,
                        "skipped",
                        f"可选登录步骤已跳过: {step_result.message}",
                        dict(step_result.payload),
                        tuple(step_result.warnings) + (step_result.message,),
                    )
        ...
        # 应用已经成功拉起但登录/港区探针尚未确认时，给 UI 一个可操作的
        # “started” 状态。这样“测试游戏启动”不会因为首帧仍在加载或 OCR
        # 暂时不可用而显示成启动失败；真正进入港区仍须由后续 OCR 再确认。
        if probe is not None and foreground_package == package:
            warnings.append("游戏进程已在前台，但尚未确认登录完成；请继续等待或由 OCR 再次检查。")
            return AdbLoginResult(
                True,
                "started",
                "游戏已启动，但尚未确认进入港区主页。",
    ...
    def _tap_text(
        ...
        *,
        ...
        raw_candidates = step.get("text_candidates", step.get("text", ()))
        ...
        if not candidates:
            return AdbCommandResult(False, "invalid_text", "tap_text 需要至少一个文本候选。")

        scene = RecognitionScene.normalize(step.get("scene", RecognitionScene.HARBOR.value))
        screenshot = self.capture_screenshot(
            scene,
            serial=serial,
            screen_state=str(step.get("screen_state", scene.value)),
            scene_hint=str(step.get("scene_hint", scene.value)),
            task_context=task_context,
        )
        if not screenshot.success or screenshot.artifact is None:
            return AdbCommandResult(False, screenshot.status, screenshot.message, warnings=screenshot.warnings)

        detector = get_screen_state_detector()
        lines = detector.ocr_engine.recognize_lines(
            screenshot.artifact.screenshot_path,
            roi=step.get("roi"),
            preprocess=bool(step.get("preprocess", True)),
        )
        if not lines:
            return AdbCommandResult(False, "not_found", "OCR 未找到可点击文本。", warnings=())

        match_mode = str(step.get("match_mode", "contains")).strip().lower()
        search_pool = []
        for candidate in candidates:
            normalized_candidate = self._normalize_text_token(candidate)
            for line in lines:
                normalized_text = self._normalize_text_token(line.text)
                if not normalized_text:
                    continue
                matched = (
                    normalized_candidate == normalized_text
                    if match_mode == "exact"
                    else normalized_candidate in normalized_text
                )
                if not matched:
                    continue
                if line.box is None:
                    continue
                search_pool.append((candidate, line))

        if not search_pool:
            return AdbCommandResult(
                False,
                "not_found",
                f"未在截图中找到候选文本: {' / '.join(candidates)}",
                warnings=(),
            )
        ...
        candidate, line = max(
            ...
            key=lambda item: (
                ...
                len(self._normalize_text_token(item[0])),
                len(self._normalize_text_token(item[1].text)),
            ),
        )
        x, y, w, h = (int(value) for value in line.box or (0, 0, 0, 0))
        if w <= 0 or h <= 0:
            return AdbCommandResult(False, "invalid_box", "OCR 文本框无效，无法点击。")
        tap_x = x + w / 2.0
        tap_y = y + h / 2.0
        tap_result = self.tap(tap_x, tap_y, serial=serial, task_context=task_context)
        if tap_result.success:
            tap_result = AdbCommandResult(
                True,
                tap_result.status,
                f"已点击文本: {candidate}",
    ...
    @staticmethod
    def _normalize_text_token(text: object) -> str:
        """把 OCR/按钮文本压成方便比较的字符串。"""
    ...
    def _load_login_steps(warnings: list[str]) -> list[Dict[str, Any]]:
        ...
        if isinstance(raw_login, dict):
            ...
            # 旧版本这里通常是示例坐标（例如 100,200 / 300,400），
            # 在真实登录页上盲点可能关闭游戏或触发不可逆操作。保留配置
            # 兼容读取能力，但默认停用，调用方若确实需要可显式传入 login_steps。
            warnings.append("检测到旧版固定坐标 login 配置，已停用以避免盲目点击。")
            raw_steps = []
        else:
