...
@dataclass(frozen=True)
class EquipmentPageRaritySweepFrame:
    """
    设计图稀有度切换过程中的单步截图摘要。
    输入：
        screenshot_path/session_id/frame_index/rarity_state 等字段。
    输出：
        供 OCR 或人工核对的稀有度切换证据。
    使用示例：
        frame = session.frames[0]
    ...
    scene: str = RecognitionScene.EQUIPMENT_LIST.value
    action_name: str = "set_rarity_filter"
    action_result: str = "ready"
    action_message: str = "设计图稀有度切换完成。"
    sha1: str = ""
...
class EquipmentPageRaritySweepSession:
    ...
    page_name: str
    page_state: str
    frames: Tuple[EquipmentPageRaritySweepFrame, ...]
    run_dir: str
    frames_dir: str
    manifest_path: str
