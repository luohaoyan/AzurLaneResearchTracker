...
class AzurLaneClientProfile:
    ...
    碧蓝航线客户端配置。
    输入：
        key/display_name/package_name/activity_name/server_group/channel。
    输出：
        可序列化字典，供 API payload 和 UI 下拉框使用。
    使用示例：
...
# 参考 LmeSzinc/AzurLaneAutoScript 的 module/config/server.py：
# - VALID_PACKAGE / VALID_CHANNEL_PACKAGE 负责包名识别；
# - DICT_PACKAGE_TO_ACTIVITY 负责不同渠道 Activity；
# - VALID_SERVER_LIST 负责服务器名称。
...
def select_azur_lane_client(package_names: Iterable[str], requested_key: str) -> Optional[AzurLaneClientProfile]:
    ...
    根据用户选择和已安装包名选择客户端。
    输入：
        package_names: ADB 扫描出的安装包。
        requested_key: official_cn / global_en / auto 等。
    输出：
        匹配的客户端；未安装所选客户端时返回 None。
    """
